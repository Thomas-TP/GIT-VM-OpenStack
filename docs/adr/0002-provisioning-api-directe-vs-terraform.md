# ADR 0002 — Provisioning par API OpenStack directe + module Terraform documenté
...
actuelle crée les VM par **appels API OpenStack directs** depuis le Worker (`fetch`). Un Cloudflare
...
1. **Le chemin live reste l'API OpenStack directe** depuis le Worker (provisioning à l'approbation).
2. Nous fournissons **en parallèle un module Terraform/OpenTofu** (`infra/`) reproduisant la même
   VM (instance, volume, SG, clé), **documenté et testé**, pour : (a) cocher l'exigence IaC au sens
