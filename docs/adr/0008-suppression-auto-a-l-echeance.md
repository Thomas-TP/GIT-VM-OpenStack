...
- `terminateInstance` (serveur OpenStack) **+ `deleteKeyPair`** (clé SSH) ;
...
- **Coût nul** après échéance : ni compute ni stockage résiduel (le volume est détruit avec
  l'instance, `delete_on_termination=true`).
...
- (+) Aucune ressource OpenStack orpheline / facturée après l'échéance.
