...
stateDiagram-v2
    ...
    active --> active: expired_at posé à end_date<br/>(VM arrêtée, données conservées)
...
par la **cron** quand l'instance tourne avec une IP. « expired » est **dérivé** de `expired_at`
(le statut DB reste `active`) — voir [ADR 0004](adr/0004-cycle-de-vie-reconciliateur.md).
...
- **`enforceExpiry`** : à `end_date` → arrête la VM + `expired_at` + email ; e-mail de pré-échéance 24 h avant.
