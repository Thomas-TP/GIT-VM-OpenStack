...
- **Backend** : Hono, pas de dépendances lourdes, appels OpenStack via `fetch` natif (`src/openstack.ts`).
...
- Nouvelle migration : `migrations/NNNN_nom.sql`. Elle sera appliquée automatiquement au déploiement
  (deploy command Cloudflare). En local : `npx wrangler d1 migrations apply git_vm_portal_openstack --local`.
